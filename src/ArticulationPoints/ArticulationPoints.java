    ...
    public static void findNeighboursForSingleOnLoad(HashMap<Coordinates, APNode> nodes, Coordinates currentLocation, ImageFile imgObj) {
        ...
        for (int y = currentLocation.getY() - 1; y > -1; y--) {
            ...
            if (!imgObj.isWhite(currentLocation.getX(), y)) {
                ...
                nodes.get(currentLocation).addNeighbour(nodes.get(new Coordinates(currentLocation.getX(), y)));
                nodes.get(new Coordinates(currentLocation.getX(), y)).addNeighbour(nodes.get(currentLocation));
                break;
            }
        }
        ...
        for (int x = currentLocation.getX() - 1; x > -1; x--) {
            ...
            if (!imgObj.isWhite(x, currentLocation.getY())) {
                ...
                //If a node is found, mark them as neighbours
                nodes.get(currentLocation).addNeighbour(nodes.get(new Coordinates(x, currentLocation.getY())));
                nodes.get(new Coordinates(x, currentLocation.getY())).addNeighbour(nodes.get(currentLocation));
                break;
            }
        }
    }
